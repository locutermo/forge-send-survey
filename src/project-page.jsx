...
const App = () => {
  ...
  const fetchSurveyData = async () => {
    ...
    try {
      ...
      const searchRes = await requestJira(
        `/rest/api/3/search?jql=project = "${projectKey}" AND "cf[12706]" is not EMPTY ORDER BY updated DESC&maxResults=50&fields=summary,customfield_12706,updated,comment`
      );
  ...
  const head = {
    cells: [
      { key: 'key', content: 'Clave Ticket', isSortable: true, width: 15 },
      { key: 'summary', content: 'Resumen', width: 35 },
      { key: 'rating', content: 'Calificación', isSortable: true, width: 15 },
      { key: 'comments', content: 'Comentarios', width: 25 },
      { key: 'updated', content: 'Fecha de Respuesta', width: 10 }
    ]
  };
  ...
  const rows = ticketList.map((item, index) => {
    ...
    return {
      ...
      cells: [
        ...
        {
          key: 'comments',
          content: <Text>{item.comments || 'Sin comentarios'}</Text>
        },
        {
          key: 'updated',
...
export const run = ForgeReconciler.render(<App />);
